...
from app.context import TaskContext, get_task_id, log_action, inspect_context, log_file_action, get_files
...
spreadsheet_analysis_agent = Agent[TaskContext](
    ...
    instructions="".join([
        ...
        "- Aim for McKinsey level presentation and clarity",
        "- Your final output musst adheer to the professional standards of a top consulting firm, void plaguing the user with bannal insight and empty pieces of details"
...
spreadsheet_agent = Agent[TaskContext](  # <--- Added TaskContext type hint
    ...
    instructions=''.join([
        ...
        "- run_spreadsheet_operation: Use this to modify existing spreadsheets (filename MUST be `[task_id]_your_filename.xlsx`).",

        "YOU HAVE A BUNCH OF ANALYSIS TOOLS AT YOUR  DISPOSAL",
        "- spreadsheet_analysis_agent: Use this to perform complex analysis operations on spreadsheets ",
        "- analyze_spreadsheet: Use this to perform simpler, controlled operations on spreadsheets ",
        "- extract_spreadsheet_from_document: Use this to extract spreadsheet data from documents (e.g., PDF, images, NEVER csv or xlsx), We can read documets and caputree thee table in them ideeal for invoices, bank statments and any job for extracting tables from documents",
        ...
        "ENHANCEMENT TOOLS",
        "- magic_enhance_excel: Use this to enhance the spreadsheet with advanced features (e.g., charts, pivot tables, formats etc, be very specific)",
        "- /magic_enhance_excel/ ca be used for modifying sheets and columns",
        "- use the mgic eenhaance tol for any dit action you wat to take on the sheet",
        "**CRITICAL RULES**: ",
    ...
    tools=[
        ...
        spreadsheet_analysis_agent.as_tool(
            tool_name="spreadsheet_analysis_agent",
            tool_description="Use this tool to hand off to the spreadsheet analysis agent for complex analysis tasks.",
        )
    ]
)
